...
def test_logicnets_verilog():
    ...
    general_output = helper.make_tensor_value_info(
        "general_output", TensorProto.FLOAT, [2]
    )
    ...
    def expected_output(input_data, indices0, indices1, care_set0, care_set1, in_bits):
        ...
        sparse_out0 = np.array(
            [concat_out[int(indices0[0])], concat_out[int(indices0[1])]]
        )
        sparse_out1 = np.array([concat_out[indices1[0]], concat_out[indices1[1]]])

        sparse_out0_int = npy_to_rtlsim_input(
            sparse_out0, DataType.BINARY, in_bits, False
        )[0]
        sparse_out1_int = npy_to_rtlsim_input(
            sparse_out1, DataType.BINARY, in_bits, False
        )[0]
